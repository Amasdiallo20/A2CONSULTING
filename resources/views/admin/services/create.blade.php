...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1><i class="fas fa-plus me-2"></i> Créer un service</h1>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('price_type') is-invalid @enderror" 
                            ...
                            <option value="fixed" {{ old('price_type') == 'fixed' ? 'selected' : '' }}>Fixe</option>
                            <option value="per_person" {{ old('price_type') == 'per_person' ? 'selected' : '' }}>Par personne</option>
                            <option value="per_session" {{ old('price_type') == 'per_session' ? 'selected' : '' }}>Par session</option>
                            <option value="custom" {{ old('price_type') == 'custom' ? 'selected' : '' }}>Sur devis</option>
                        </select>
                        @error('price_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('icon') is-invalid @enderror" 
                               id="icon" name="icon" value="{{ old('icon') }}" placeholder="ex: fa-lightbulb-o">
                        @error('icon')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
